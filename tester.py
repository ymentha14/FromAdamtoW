...
class Tester:
    ...
    def _run_one_epoch(self, dataset: torch.utils.data.DataLoader, criterion: _WeightedLoss, optimizer: torch.optim):
        """
    ...
    def cross_validation(self, k: int = 5, test_split: float = 0.1):
        ...
        train_indices, val_indices = np.array(indices[split:]), np.array(indices[:split])
        ...
        for cv in range(k):
            ...
            test_indices_cv = np.concatenate([indices[:lower_extreme], indices[higher_extreme:]])
            # else:
            ...
            train_loader_cv = torch.utils.data.DataLoader(train_dataset_cv, batch_size=self.batch_size)
            test_loader_cv = torch.utils.data.DataLoader(test_dataset_cv, batch_size=self.batch_size)
            ...
            for epoch in range(num_epochs):
                ...
                print("The score at epoch {} is {}".format(
                    str(epoch+1),
                    self.score(self.model, test_loader_cv)))
            print('\n')
