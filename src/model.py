...
def load_most_recent(model):
    ...
    file_name = max(
        [
            file
            for root, dirs, files in os.walk(WEIGHT_DIR, topdown=False)
            for file in files
        ]
    )
...
def train_model_noval(
    model, X_train, y_train, nb_epochs=5, lr=2e-4, optimz=optim.AdamW
):
    """
    trains the model passed in attribute
    Args:
...
def compute_pred(file_name):
    ...
    return {"true_label": target, "predicted": predicted}
...
def callback(model, X_train, X_test, history, criterion):
    ...
    print(
        "Train: {:.2f}% Validation: {:.2f}% ".format(
            hist["accuracy_score"] * 100, hist["valid_acc"] * 100
        )
    )
    history.append(hist)
